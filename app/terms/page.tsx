import { CONTACT_EMAIL } from '@util/global';

import styles from '@styles/pages/file.module.css';



export default function Page() {
    return (
        <div className={styles.container}>
            <p>Last Updated: January 24, 2025</p>

            <h1>Terms of Service</h1>





            <h2>1. Acceptance of Terms</h2>

            <p>Please read these Terms of Service (&quot;Terms&quot;, &quot;Terms of Service&quot;) carefully before using UniVerse (the &quot;Service&quot;) operated by UniVerse (&quot;us&quot;, &quot;we&quot;, or &quot;our&quot;).</p>

            <p>By accessing this Service, you (&quot;User&quot;, &quot;you&quot;, or &quot;your&quot;) are agreeing to be bound by these Terms of Service, all applicable laws and regulations, and agree that you are responsible for compliance with any applicable local laws.</p>



            <h2>2. Eligibility</h2>

            <h3>2.1 Basic Requirements</h3>
            <p>The Service is intended for use by university students. By using the Service, you represent and warrant that:</p>
            <ul>
                <li>You are currently enrolled at an accredited university</li>
                <li>You have a valid university email address</li>
                <li>You are at least 18 years old</li>
                <li>You have the right, authority, and capacity to accept these Terms</li>
            </ul>

            <h3>2.2 University Affiliation</h3>
            <p>The Service is designed for use by university students and requires a valid university email address (.edu domain) for registration. You agree to provide accurate information about your university affiliation. We reserve the right to verify your university status and terminate accounts that misrepresent their affiliation.</p>

            <h3>2.3 FERPA Compliance</h3>
            <p>You acknowledge that while this Service is designed for university students, it is not an official university service and is not subject to the Family Educational Rights and Privacy Act (FERPA). However, we encourage users to respect educational privacy principles.</p>



            <h2>3. User Account</h2>

            <h3>3.1 Registration</h3>
            <p>You agree to:</p>
            <ul>
                <li>Register for an account using your university email address</li>
                <li>Provide accurate, current, and complete information</li>
                <li>Being responsible for maintaining the confidentiality of your account credentials</li>
                <li>Immediately notify us of any unauthorized use of your account</li>
            </ul>

            <h3>3.2 Account Restrictions</h3>
            <p>You agree not to:</p>
            <ul>
                <li>Create or use multiple accounts</li>
                <li>Sell or transfer your account</li>
                <li>Create an account using another person&apos;s identity</li>
            </ul>

            <h3>3.2 Account Moderation</h3>
            <p>We reserve the right to suspend or terminate accounts at our discretion.</p>



            <h2>4. User Content</h2>

            <h3>4.1 Ownership</h3>
            <p>When you submit, post, or display content on or through the Service, you grant us a worldwide, non-exclusive, royalty-free license (with the right to sublicense) to use, copy, reproduce, process, adapt, modify, publish, transmit, display, and distribute such content in any and all media or distribution methods now known or later developed. This license authorizes us to make your content available to the rest of the world and to let others do the same.</p>
            <p>You agree that this license includes the right for us to:</p>
            <ul>
                <li>Provide, promote, and improve the Service</li>
                <li>Make content submitted to or through the Service available to other companies, organizations, or individuals for the syndication, broadcast, distribution, promotion, or publication of such content</li>
                <li>Use the content for marketing or promotional purposes</li>
                <li>Analyze, modify, or adapt the content as needed to transmit, display, or distribute it</li>
                <li>Remove metadata associated with content</li>
                <li>Make incidental copies as necessary to deliver your content</li>
                <li>Access, store, and scan your content</li>
            </ul>
            <p>You retain ownership of content you post on the Service. By posting content, you grant us a worldwide, non-exclusive, royalty-free license to use, copy, modify, distribute, and display your content.</p>
            <p>You agree that your content will be viewable by other users of the Service and that other users may interact with, share, or engage with your content as permitted by the Service&apos;s functionality (such as through likes or replies).</p>

            <h3>4.2 Prohibited Content</h3>
            <p>You agree not to post prohibited content. Prohibited content includes but is not limited to:</p>
            <ul>
                <li>Is illegal, fraudulent, or misleading</li>
                <li>Infringes on intellectual property rights</li>
                <li>Contains hate speech or discriminatory content</li>
                <li>Harasses, bullies, or threatens others</li>
                <li>Contains explicit or pornographic material</li>
                <li>Promotes violence or illegal activities</li>
                <li>Contains malware or malicious code</li>
                <li>Spam or unauthorized commercial content</li>
            </ul>

            <h3>4.3 Content Moderation</h3>
            <p>We reserve the right to delete content at our discretion.</p>

            <h3>4.4 Third-Party Links</h3>
            <p>The Service may contain links to third-party websites. We are not responsible for the content or practices of any linked websites.</p>



            <h2>5. Community Guidelines</h2>

            <p>Users must:</p>
            <ul>
                <li>Respect other users&apos; privacy and personal information</li>
                <li>Report inappropriate content or behavior</li>
                <li>Follow university-specific guidelines and policies</li>
                <li>Maintain civil discourse and respect diverse viewpoints</li>
            </ul>
            <p>You agree not to:</p>
            <ul>
                <li>Use the service for any unlawful purpose</li>
                <li>Harass, abuse, or harm others</li>
                <li>Impersonate others or provide false information</li>
                <li>Attempt to gain unauthorized access to the Service</li>
                <li>Interfere with or disrupt the Service</li>
                <li>Create multiple accounts</li>
                <li>Sell or transfer your account</li>
                <li>Use automated systems or bots</li>
            </ul>


            <h2>6. Privacy and Data</h2>

            <h3>6.1 Data Collection</h3>
            <p>We collect and process user data as described in our <a target='_blank' href='./privacy-policy'>Privacy Policy</a>. By using the Service, you consent to such processing.</p>

            <h3>6.2 Data Security</h3>
            <p>While we implement reasonable security measures, we cannot guarantee absolute security. You acknowledge this risk when using the Service.</p>



            <h2>7. Termination</h2>

            <h3>7.1 Account Termination</h3>
            <p>We reserve the right to suspend or terminate your account for:</p>
            <ul>
                <li>Violation of these Terms</li>
                <li>Engaging in prohibited activities</li>
                <li>Extended periods of inactivity</li>
                <li>At our sole discretion</li>
            </ul>

            <h3>7.2 Effect of Termination</h3>
            <p>Upon termination:</p>
            <ul>
                <li>Your access rights cease immediately</li>
                <li>We retain your data in our systems</li>
                <li>We may delete your content and account information</li>
                <li>Relevant sections of these Terms remain in effect</li>
            </ul>



            <h2>8. Disclaimers and Limitations</h2>

            <h3>8.1 Service Availability</h3>
            <p>You understand and agree that:</p>
            <ul>
                <li>The Service is provided &quot;as is&quot; and &quot;as available&quot; without warranties of any kind</li>
                <li>We do not guarantee uninterrupted or error-free service</li>
                <li>We may modify or discontinue features without notice</li>
            </ul>

            <h3>8.2 Limitation of Liability</h3>
            <p>You understand that:</p>
            <ul>
                <li>We are not liable for any indirect, incidental, special, consequential, or punitive damages</li>
                <li>We are not liable for any indirect, incidental, or consequential damages</li>
                <li>Our total liability shall not exceed the amount you paid us, if any</li>
                <li>You agree to defend and indemnify us against third-party claims</li>
            </ul>



            <h2>9. Intellectual Property</h2>

            <h3>9.1 Service Content</h3>
            <p>You understand and agree that:</p>
            <ul>
                <li>All Service content, features, and functionality are owned by us</li>
                <li>You may not copy, modify, or create derivative works without permission</li>
            </ul>

            <h3>9.2 Trademarks</h3>
            <p>You understand and agree that:</p>
            <ul>
                <li>All trademarks, logos, and service marks are our property</li>
                <li>You may not use them without prior written permission</li>
            </ul>



            <h2>10. Changes to Terms</h2>
            <p>We reserve the right to modify these Terms at any time. We will notify users of significant changes through:</p>
            <ul>
                <li>In-app notifications</li>
                <li>Email notifications</li>
                <li>Service announcements</li>
            </ul>
            <p>Continued use of the Service after any changes constitutes acceptance of the new Terms.</p>



            <h2>11. Governing Law and Jurisdiction</h2>

            <h3>11.1 Choice of Law</h3>
            <p>These Terms shall be governed by and construed in accordance with the laws of the State of Delaware, without regard to its conflict of law provisions.</p>

            <h3>11.2 Dispute Resolution</h3>
            <p>Any dispute arising from or relating to these Terms shall be resolved exclusively in the state or federal courts located in Delaware. You consent to the personal jurisdiction of such courts and waive any objection to proceedings in such courts.</p>

            <h3>11.3 Class Action Waiver</h3>
            <p>You agree that any proceedings to resolve or litigate any dispute will be conducted solely on an individual basis. You agree not to seek to have any dispute heard as a class action, representative action, collective action, or private attorney general action.</p>

            <h3>11.4 Time Limitation</h3>
            <p>You agree that any claim arising out of or related to these Terms must be filed within one (1) year after such claim arose; otherwise, the claim is permanently barred.</p>



            <h2>12. Contact Information</h2>

            <p>For questions about these Terms, contact us at: <a href={`mailto:${CONTACT_EMAIL}`}>{CONTACT_EMAIL}</a>.</p>



            <h2>13. Severability</h2>

            <p>If any provision of these Terms is found to be unenforceable, the remaining provisions will remain in effect.</p>



            <h2>14. Entire Agreement</h2>

            <p>These Terms constitute the entire agreement between you and us regarding the Service, superseding any prior agreements.
            By using our Service, you acknowledge that you have read and understood these Terms of Service and agree to be bound by them.</p>
        </div>
    );
}